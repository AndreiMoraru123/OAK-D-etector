...
def random_crop(image, boxes: torch.Tensor, labels: torch.Tensor, difficulties: torch.Tensor) -> tuple:
    """
    ...
    while True:
        ...
        # if no cropping, return the original image
        if min_overlap is None:
            return image, boxes, labels, difficulties

        # try up to 50 times to get a successful crop with a minimum overlap
        max_trials = 50
        for _ in range(max_trials):
            ...
            crop_h = int(scale_h * original_h)
            crop_w = int(scale_w * original_w)
            ...
            # Crop coordinates must be within the original image
            left = random.randint(0, original_w - crop_w)
            right = left + crop_w
            top = random.randint(0, original_h - crop_h)
            bottom = top + crop_h
            ...
            # Crop image
            image = image[:, top:bottom, left:right]  # (3, crop_h, crop_w)
            ...
            centers_in_crop = (bb_centers[:, 0] > left) * (bb_centers[:, 0] < right) * \
                              (bb_centers[:, 1] > top) * (bb_centers[:, 1] < bottom)  # (n_objects)
            ...
            new_boxes = boxes[centers_in_crop, :]  # (n_objects, 4)
            new_labels = labels[centers_in_crop]  # (n_objects)
            new_difficulties = difficulties[centers_in_crop]  # (n_objects)
            ...
            return image, new_boxes, new_labels, new_difficulties
...
def flip(image, boxes: torch.Tensor) -> tuple:
    """
...
def resize(image, boxes: torch.Tensor, dims=(300, 300), return_percent_coords=True) -> tuple:
    """
    Resize image. For the SSD300, resize to (300, 300).
    Since percent/fractional coordinates are calculated for the bounding boxes (w.r.t image dimensions) in this process,
    you may choose to retain them.
    :param dims:  (width, height)
    :param return_percent_coords:  if True, returns fractional coordinates, else returns boundary coordinates
    :param image: image, a PIL Image
    :param boxes: bounding boxes in boundary coordinates, a tensor of dimensions (n_objects, 4)
...
def transform(image, boxes: torch.Tensor, labels: torch.Tensor, difficulties: torch.Tensor, split: str) -> tuple:
    """
